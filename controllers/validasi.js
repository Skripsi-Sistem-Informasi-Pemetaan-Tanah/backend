...
export const validasiOnProgress = async (req, res) => {
    ...
    try {
        const editValidasi = await client.query("UPDATE maps SET status = 1, komentar = 'Data masih dalam progress', updated_at = NOW() WHERE map_id = $1", [mapId]);
...
export const validasiKoordinatBerhasil = async (req, res) => {
    ...
    const {mapId} = req.body;
    try {
        const editValidasi = await client.query("UPDATE koordinat SET status = 'Data Tervalidasi', komentar = 'Data Tervalidasi', progress = 100, updated_at = NOW() WHERE map_id = $1", [mapId]);
...
export const validasiDitolak = async (req, res) => {
    ...
    const {mapId} = req.body;
    try {
        const editValidasi = await client.query("UPDATE maps SET komentar = 'Lahan ditolak silahkan submit ulang', status = 3, updated_at = NOW() WHERE map_id = $1", [mapId]);
        client.release();
        if (editValidasi) return utilMessage(res, 200, "Data dengan id " + mapId + " berhasil ditolak");
...
export const cekKoordinatIDtoVerif = async (req, res) => {
    ...
    try {
        const fixedKoordinatResult = await client.query("SELECT koordinat, map_id FROM koordinat WHERE koordinat_id = $1", [koordinatId]);
        ...
        for (const koordinat of findKoordinat) {
            const distance = calculateDistance(fixedKoordinat.latitude, fixedKoordinat.longitude, koordinat.latitude, koordinat.longitude);
        ...
        await client.query("UPDATE koordinat SET koordinat_id_need_verif = $1 WHERE koordinat_id = $2", [closestKoordinatIds, koordinatId]);
        ...
        //   [formattedKoor]
        // );
        const findKoordinatWithaSameVerif = await client.query("SELECT koordinat_id_need_verif, koordinat_id FROM koordinat WHERE koordinat_verif = $1 and koordinat_id != $2", [formattedKoor, koordinatId]);
        ...
        if (findKoordinatWithaSameVerif.rows.length > 0) {
            for (const koordinatidneedverif of findKoordinatWithaSameVerif.rows) {
                ...
                if (index !== -1) {
                    ...
                    await client.query("UPDATE koordinat SET koordinat_id_need_verif = $1 WHERE koordinat_id = $2", [koordinatidneedverif.koordinat_id_need_verif, koordinatidneedverif.koordinat_id]);
                    // Get map id untuk mengubah status lahan menjadi 1
                    const getMapId = await client.query("SELECT map_id FROM koordinat WHERE koordinat_id = $1", [koordinatidneedverif.koordinat_id]);
                    await client.query("UPDATE maps SET status = 1 WHERE map_id = $1", [getMapId.rows[0].map_id]);
                }
            }
        }
...
// komentar = 'Lahan perlu divalidasi kembali oleh pemilik lahan yang bersinggungan dengan lahan Anda.'
export const cekMapIDtoVerif = async (req, res) => {
    ...
    try {
        ...
        // Query untuk mengambil semua koordinat yang akan dibandingkan jaraknya
        const findKoordinatResult = await client.query("SELECT map_id,koordinat FROM koordinat WHERE map_id != $1", [fixedKoordinat.mapId]);
        const findKoordinat = findKoordinatResult.rows.map(row => ({
            map_id: row.map_id, longitude: row.koordinat[0], latitude: row.koordinat[1]
        }));
        let distances = [];
        // Hitung jarak dari koordinat yang diberikan ke setiap koordinat dari lahan lain
        for (const koordinat of findKoordinat) {
        ...
        // Update tabel koordinat dengan closestMapIds
        await client.query("UPDATE koordinat SET map_id_need_verif = $1 WHERE koordinat_id = $2", [closestMapIds, koordinatId]);
...
const checkArround = async (mapId) => {
    try {
        ...
        const fixedKoordinatResult = await client.query("SELECT koordinat FROM koordinat WHERE map_id = $1", [mapId]);
        const fixedKoordinat = fixedKoordinatResult.rows.map(row => ({
            longitude: row.koordinat[0], latitude: row.koordinat[1]
        }));

        // Query untuk mengambil semua koordinat yang akan dibandingkan jaraknya
        const findKoordinatResult = await client.query("SELECT koordinat FROM koordinat WHERE map_id != $1", [mapId]);
        const findKoordinat = findKoordinatResult.rows.map(row => ({
            longitude: row.koordinat[0], latitude: row.koordinat[1]
        }));
...
const updateProgress = async (mapId, newProgress) => {
    try {
        ...
        await client.query("UPDATE maps SET progress = $1 WHERE map_id = $2", [newProgress, mapId,]);
        client.release();
    } catch (error) {
        // Tangani kesalahan jika terjadi saat menjalankan query
...
export const addKomentar = async (req, res) => {
    try {
        ...
        const addKomentar = await client.query("UPDATE maps SET komentar = $1 WHERE map_id = $2", [Komentar, mapId,]);
        client.release();
        if (addKomentar) {
...
const addKomentars = async (mapId, Komentar) => {
    try {
        ...
        // Query untuk mengupdate komentar pada tabel maps
        await client.query("UPDATE maps SET komentar = $1 WHERE map_id = $2", [Komentar, mapId,]);
        client.release();
    } catch (error) {
        // Tangani kesalahan jika terjadi saat menjalankan query
...
export const countPercentOfAgree = async (req, res) => {
    try {
        ...
        const {mapId} = req.body;
        // Query untuk mengupdate komentar pada tabel maps
        const dataKoor = await client.query("SELECT koordinat.status,koordinat.koordinat_id_need_verif FROM koordinat WHERE map_id = $1", [mapId]);
        let totalAgree = []
        for (const dataKoord of dataKoor.rows) {
            ...
            for (const koordinatId of filteredArray) {
                const dataKoorID = await client.query("SELECT koordinat.status FROM koordinat WHERE koordinat_id = $1", [koordinatId]);
...
export const updateStatusLahan = async (req, res) => {
    try {
        ...
        const {koor} = req.body;
        // Query untuk mengupdate komentar pada tabel maps
        const dataKoor = await client.query("SELECT map_id, koordinat_id FROM koordinat WHERE koordinat_verif = $1", [koor]);
        for (const dataKoord of dataKoor.rows) {
            // Filter out null or undefined elements
...
export const addKomentarKoordinat = async (req, res) => {
    try {
        ...
        const addKomentar = await client.query("UPDATE koordinat SET komentar = $1 WHERE koordinat_id = $2", [Komentar, koordinatId,]);
        client.release();
        if (addKomentar) {
            return utilMessage(res, 200, "Komentar berhasil ditambahkan");
        } else {
...
const addKomentarKoordinats = async (mapId, Komentar) => {
    try {
        ...
        // Query untuk mengupdate komentar pada tabel maps
        await client.query("UPDATE koordinat SET komentar = $1 WHERE map_id = $2", [Komentar, mapId,]);
        client.release();
    } catch (error) {
        // Tangani kesalahan jika terjadi saat menjalankan query
        console.error("Error while updating comment:", error);
