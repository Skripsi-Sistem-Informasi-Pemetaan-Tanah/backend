...
export const register = async (req, res) => {
    try {
        ...
        const postUser = await User.create({
            username: username, email: email, password: hashPassword, user_id: username, role: 2
        });
...
export const login = async (req, res) => {
    try {
        ...
        const accessToken = jwt.sign({userId, username: userUsername, userPassword}, process.env.PRIVATE_KEY, {
            expiresIn: "600s",
        },);
        ...
        res.cookie("refreshToken", refreshToken, {
            httpOnly: true, maxAge: 60 * 60 * 24 * 1000, secure: true, // Ensure secure flag is set only in production
